...
"don't hmm" — a Next.js 15 (App Router) + Tailwind web app backed by Supabase (Postgres). It's a counter UI: sign in with Kakao and buttons appear for logging annoying behaviors of others ("actors"), incrementing a count each tap. Four pages: 메인 (counter, `/`, swipe left/right to browse other people's pages read-only), 현황 (charts, `/stats`), 게임 (30-second minigame, `/game`), 랭킹 (three cumulative leaderboards, `/ranking`), 관리 (PIN-gated admin, `/manage`), plus `/auth/callback` (OAuth code exchange only) and the server-side routes `/api/cron/tick` and `/api/cheer` (see below). **The whole site sits behind Kakao login** — `AppShell` renders `LoginScreen` instead of the shell when signed out, so `/manage` now needs login *and* the PIN. Deployed to Vercel.
...
RLS is on for all four tables. `users`/`items` allow anon `select` only — no write policies exist, so all mutation goes through `security definer` RPC functions: `increment_item` and `set_item_style` (no PIN, but ownership-checked — each only updates a row whose `items.user_id` matches the caller's `users.auth_user_id = auth.uid()`, so only the item's own owner can tap it or restyle it; granted to `authenticated` only, not `anon`), `add_my_item` / `delete_my_item` / `update_my_item` (same ownership check, let the signed-in owner add/remove/rename their own items without the PIN), and `admin_verify_pin` / `admin_upsert_user` / `admin_delete_user` / `admin_upsert_item` / `admin_delete_item` (each takes `p_pin` and calls an internal `check_pin_()` helper that raises if it doesn't match `app_config.admin_pin` — this is the one path that can still touch *anyone's* items). This means both the PIN and the ownership checks are enforced in the database, not just the UI — a leaked `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` alone can't mutate data, and a signed-in user can't mutate someone else's items either. The `item-backgrounds` storage bucket mirrors this: its upload policy checks that the uploaded path's item id belongs to the caller before allowing the write. `daily_counts(p_user_id, p_days)` aggregates `events` per day (KST) for the stats line chart.
...
- `src/components/NavIcon.tsx` — the bottom tab bar / hamburger menu icons, picked by `NAV` key. Inline stroke SVGs in the same thin-line style as `HmmFace`; add a new entry here when adding a nav item, or the tab renders label-only.
...
- Optimistic increment in `HabitCounter`: bump local count immediately, call `increment_item`, reconcile with the server's authoritative count (handles concurrent taps from your own other tabs/devices — `increment_item` rejects anyone else's), roll back on failure. A `pending` ref count suppresses the poll loop from clobbering in-flight optimistic state. Tapping no longer forces an immediate full `fetchSnapshot` refetch afterward (that used to reshuffle the whole `rows` array on every tap); the rest of the state just catches up on the next `POLL_MS` tick.
...
Three awards, all **most-first** and all sarcastic, same as the minigame: **가장 부지런한 동료를 둔 친구** (sum of `items.count` per user, `total_leaderboard()`), **목표 지향적인 동료를 둔 친구** (cumulative minigame taps, reusing `game_leaderboard()`), and **곧 열반에 오를 친구들** (most `cheers` *received*, `cheer_leaderboard()` — the more "힘내요" you've gotten, the more noise you've had to endure). A high score means your colleagues were noisy, not that you did well — keep the copy ironic.
...
`Fanfare` fires once on mount when `is_me` lands in the top `FANFARE_RANK` of *any* of the three boards (the better rank wins; it never fires twice). The sound is synthesized with `OscillatorNode` rather than shipping an audio asset, and the whole thing is wrapped in try/catch — **if audio is blocked the confetti must still run.** Arriving via a nav tap carries user activation so the sound plays; a cold load straight to `/ranking` may be silent. Confetti is portalled to `document.body` (the header's `backdrop-blur` traps `fixed`) and hidden under `prefers-reduced-motion`.
...
`src/app/api/cron/tick/route.ts` (Node runtime, `CRON_SECRET` bearer, secret key). It is **idempotent by design** so the driving scheduler can tick at any interval: create today's round if missing, send the push if `start_at` has passed and `notified_at` is null, otherwise do nothing. It stamps `notified_at` *before* sending, via a conditional update on `is null`, so overlapping ticks cannot double-send. KST boundaries are computed with an explicit +9h offset because the server runs on UTC.

`src/app/api/cheer/route.ts` is the other server-side route: the 메인 page's "💪 힘내요" button (shown only when viewing someone else's page — "hang in there, putting up with all this noise") POSTs here with the viewer's Supabase access token and the target `users.id`. The route verifies the token via `supabase.auth.getUser()` (anon key), then does everything else — the ownership check, a 10-minute per-sender-per-target cooldown, the `cheers` insert, and the web push send — with the secret key, mirroring `/api/cron/tick`'s pattern. `cheers` is RLS-on/no-policies like `game_rounds`; the only client-facing read of it is the aggregate `cheer_leaderboard()` RPC that powers 랭킹's third board, "곧 열반에 오를 친구들" (most cheers *received* — sarcastic, same as the other two). `public/sw.js`'s push handler takes `payload.tag` per notification type now (defaults to `"dont-hmm"`) so a cheer notification doesn't collide with/replace a pending game-start notification.
